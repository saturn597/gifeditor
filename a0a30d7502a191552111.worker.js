...
function canvasDataToGIFData(canvasData, width, height) {
    ...
    var _getColorData = getColorData(colors),
        colorTable = _getColorData.colorTable,
        encodedColorTableSize = _getColorData.encodedColorTableSize,
        minCodeSize = _getColorData.minCodeSize;
    ...
    var cr = getCodeReader(indices, minCodeSize);
    ...
    return {
        ...
        encodedColorTableSize: encodedColorTableSize,
...
var Frame = function () {
    ...
    function Frame(imageData, delay, disposal) {
    ...
    _createClass(Frame, [{
        ...
        value: function getData() {
            ...
            var _imageData = this.imageData,
                colorTable = _imageData.colorTable,
                encodedColorTableSize = _imageData.encodedColorTableSize,
                minCodeSize = _imageData.minCodeSize,
                transparentIndex = _imageData.transparentIndex,
            ...
            (_result = result).push.apply(_result, _toConsumableArray(getGCE(this.delay, this.disposal, transparentIndex)));

            // Push the image descriptor section
            (_result2 = result).push.apply(_result2, [0x2c].concat(_toConsumableArray(getBytes(imageLeft, 2)), _toConsumableArray(getBytes(imageTop, 2)), _toConsumableArray(getBytes(width, 2)), _toConsumableArray(getBytes(height, 2)), [pack([[colorTable.length > 0 ? 1 : 0, 1], // Local color table present?
            ...
            if (colorTable.length > 0) {
                ...
                (_result3 = result).push.apply(_result3, _toConsumableArray(colorTable));
            }
        ...
        value: function setDelay(delay) {
            if (!Number.isInteger(delay) || delay < 0 || delay >= Math.pow(256, 2)) {
        ...
        key: 'setDisposal',
        value: function setDisposal(disposal) {
...
function getGifData(frames, repeats, width, height) {
    ...
    var globalColors = []; // Currently not using global color table
    // TODO: might be good to have one for some cases?

    var _getColorData2 = getColorData(globalColors),
        colorTable = _getColorData2.colorTable,
        encodedColorTableSize = _getColorData2.encodedColorTableSize;

    var bgColorIndex = 0; // index to use for pixels not specified in image data. 0 if no global color table.

    // http://stackoverflow.com/questions/7128265/purpose-of-color-resolution-bits-in-a-gif
    // (I'm not really setting this correctly).
    var colorResolution = encodedColorTableSize;

    var pixelAspectRatio = 0;

    // First 6 bytes identify this as a GIF, version 89a
    var data = toCharCodes('GIF89a');

    // Now the logical screen descriptor.
    (_data = data).push.apply(_data, _toConsumableArray(getBytes(width, 2)).concat(_toConsumableArray(getBytes(height, 2)), [pack([[colorTable.length > 0 ? 1 : 0, 1], // global color table present?
    [colorResolution, 3], [0, 1], // this is the sort flag, which is now largely ignored
    [encodedColorTableSize, 3]]), bgColorIndex, pixelAspectRatio]));

    // Add global color data if present.
    if (colorTable.length > 0) {
        var _data2;

        (_data2 = data).push.apply(_data2, _toConsumableArray(colorTable));
    }
    ...
    if (frames.length > 1) {
        ...
        (_data3 = data).push.apply(_data3, [0x21, // Extension introducer.
        0xff, // Labels this as an application extension.
        0x0b].concat(_toConsumableArray(toCharCodes('NETSCAPE2.0')), [0x03, // Indicates 3 bytes of data follow.
        0x01], _toConsumableArray(getBytes(repeats, 2)), [// Number of repeats.
...
function getGifUrl() {
    ...
    var DATAURLPREFIX = 'data:image/gif;base64,';
    var data = getGifData.apply(undefined, arguments);
    ...
    return DATAURLPREFIX + window.btoa(data.map(function (i) {
...
function getByteReader(items, outSize) {
    ...
    reader[Symbol.iterator] = regeneratorRuntime.mark(function _callee() {
        var _iteratorNormalCompletion4, _didIteratorError4, _iteratorError4, _iterator4, _step4, _ref, _ref2, code, size;
            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    ...
                    case 5:
                        ...
                        currentPiece += code * Math.pow(2, currentPieceSize);
                        currentPieceSize += size;

                    case 12:
                        if (!(currentPieceSize >= outSize)) {
                            _context.next = 19;
                            break;
                        }

                        _context.next = 15;
                        return currentPiece & Math.pow(2, outSize) - 1;
        ...
        }, _callee, this, [[3, 24, 28, 36], [29,, 31, 35]]);
    });
...
function getCodeReader(indices, minCodeSize) {
    ...
    function initializeCodeTable() {
        ...
        for (var i = 0; i < Math.pow(2, minCodeSize); i++) {
            codeTable.set(i.toString(10), i);
        }
        codeTable.set('clear', Math.pow(2, minCodeSize));
        codeTable.set('end', Math.pow(2, minCodeSize) + 1);
        nextCode = codeTable.get('end') + 1;
    }
    initializeCodeTable();

    var reader = {};
    reader[Symbol.iterator] = regeneratorRuntime.mark(function _callee2() {
        var indexBuffer, indexBufferString, _iteratorNormalCompletion5, _didIteratorError5, _iteratorError5, _iterator5, _step5, index;
            ...
            while (1) {
                switch (_context2.prev = _context2.next) {
                    case 0:
                        ...
                        indexBufferString = indices[0].toString(10);
                        _context2.next = 4;
                        return [codeTable.get('clear'), codeSize];
                    ...
                    case 4:
                        ...
                        _context2.prev = 7;
                        _iterator5 = indices.slice(1)[Symbol.iterator]();
                    ...
                    case 9:
                        ...
                        _context2.next = 16;
                        return [codeTable.get(indexBuffer.slice(0, -1).join(',')), codeSize];
                    ...
                    case 42:
                        _context2.next = 44;
                        return [codeTable.get(indexBuffer.join(',')), codeSize];

                    case 44:
                        _context2.next = 46;
                        return [codeTable.get('end'), codeSize];
        ...
        }, _callee2, this, [[7, 30, 34, 42], [35,, 37, 41]]);
    });
...
function getColorData(colors) {
    ...
    // all of the colors?
    var baseCodeSize = Math.ceil(Math.log2(colors.length));
    ...
    // with one bit).
    var minCodeSize = Math.max(2, baseCodeSize);
    ...
    var actualColorCount = Math.pow(2, baseCodeSize);
    ...
    var colorTable = (_ref3 = []).concat.apply(_ref3, _toConsumableArray(colors.concat.apply(colors, _toConsumableArray(padding))));
...
function getGCE(delay, disposal, transparentIndex) {
    ...
    return [0x21, // Begin header. 0x21 marks the beginning of an extension block.
    0xf9, // Identifies this block as a GCE.
    0x04, // Header ends with number of bytes that follow.
    pack([[0, 3], // Reserved for future use in GIF standard.
    [disposal, 3], // Disposal method gets 3 bits.
    [0, 1], // "User input flag" - not widely used, just 0.
    [transparency ? 1 : 0, 1]])].concat(_toConsumableArray(getBytes(delay, 2)), [// 2 bytes for the delay time.
...
function toIndices(d) {
    ...
    var transparentIndex = void 0;

    // GIF pixels must be fully transparent or fully opaque.
    var allowedOpacities = [0, 255];
    var opacityWarningShown = false;
    ...
    for (var i = 0; i < d.length; i += 4) {
        ...
        var current = [d[i], d[i + 1], d[i + 2]];

        var index = colorMap.get(current.join(','));
    ...
    return { indices: indices, colors: colors, transparentIndex: transparentIndex };
}
...
function getBytes(integer, numBytes) {
    ...
    if (integer > Math.pow(2, 8 * numBytes) - 1) {
        throw 'Insufficient bytes for number given.';
    }

    var byteMask = 255; // i.e., 11111111
    var output = [];
...
function pack(data) {
    ...
    try {
        for (var _iterator6 = data.reverse()[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
            ...
            if (!Number.isInteger(value) || value < 0 || value >= Math.pow(2, size)) {
                throw "Can't pack value into byte";
            }
            b += value * Math.pow(2, currentPosition);
...
var _gifs = __webpack_require__(114);
...
onmessage = function onmessage(e) {
    ...
    postMessage((0, _gifs.canvasDataToGIFData)(data, width, height));
};
